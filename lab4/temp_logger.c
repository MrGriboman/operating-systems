...
#else
#define USB_DEV_READ "/dev/pts/6"
#endif

#ifdef _WIN32
HANDLE open_com_port(const char *port_name) {
  HANDLE hComm =
      CreateFileA(port_name, GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...
void load_pass(char *db_password) {
  FILE *conf_file = fopen("conf.txt", "r");
  if (conf_file == NULL) {
    perror("ERROR: couldn't open .conf file\n");
    exit(EXIT_FAILURE);
  }
  fgets(db_password, 100, conf_file);
}
#endif
